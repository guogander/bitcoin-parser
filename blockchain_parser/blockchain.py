...
# Constant separating blocks in the .blk files  .blk文件中的常量分隔块
...
def get_files(path):
    ...
    files contained in that directory给定.bitcoin目录的路径，返回该目录中包含的.blk文件的排序列表
    """
...
def get_blocks(blockfile):
    ...
    yields its raw hexadecimal value 给定.blk文件的名称，对于该文件中包含的每个块，将生成其原始十六进制值
    """
...
def get_block(blockfile, offset):
    ...
    从块文件中以给定偏移量提取单个块"""
    with open(blockfile, "rb") as f:
        f.seek(offset - 4)  # Size is present 4 bytes before the db offset 大小在db偏移量之前4个字节
...
class Blockchain(object):
    ...
    def get_unordered_blocks(self):
        ...
        without ordering them according to height.按原样生成.blk文件中包含的块，而不按高度排序
        """
    ...
    def __getBlockIndexes(self, index):
        ...
        leveldb没有关闭数据库（并释放锁）的方法，这会在并发期间产生问题操作。这个函数还提供索引缓存。
        """
    ...
    def _index_confirmed(self, chain_indexes, num_confirmations=6):
        ...
        检查“chain_indexes”中的第一个块索引是否至少有“num_confirmation”（6）个块建立在上面它。
        如果不，这还没有得到证实，是个孤块。
        """
    ...
    def get_ordered_blocks(self, index, start=0, end=None, cache=None):
        ...
        根据bitcoind维护的路径索引中leveldb索引的高度提取，生成.blk文件中包含的块
        """

        blockIndexes = None

        if cache and os.path.exists(cache):
            # load the block index cache from a previous index 从以前的索引加载块索引缓存
            with open(cache, 'rb') as f:
                blockIndexes = pickle.load(f)  #打开文件使用pickle.load后，生成的文件后续可以一直调用

        if blockIndexes is None:
            # build the block index 建立块索引
            blockIndexes = self.__getBlockIndexes(index)
            if cache and not os.path.exists(cache):
                # cache the block index for re-use next time 缓存块索引以便下次重用
                with open(cache, 'wb') as f:
                    pickle.dump(blockIndexes, f) #pickle.dump(obj, f）：将对象obj保存到文件f中去
