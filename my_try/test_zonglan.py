...
for blocks in blockchains.get_unordered_blocks():
    for tx in blocks.transactions:
        for outputno, output in enumerate(tx.outputs):
            ...
            #
            print \
                ("blk=%s,n_tx=%d,txid=%s hash=%s vsize=%d  n_inputs=%d n_outputs=%d outputno=%d type=%s value=%s output_add=%s" % (
            ...
            resultFyle = open("blocks_hash.csv",'wb')
            wr = csv.writer(resultFyle,dialect ='excel')
            wr.writerow(blocks.hash)


            # blocks.hash.to_excel('/home/guoyaqun/python-bitcoin-blockchain-parser-master/my_try/blocks_hash.xls')
